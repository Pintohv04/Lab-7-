...
#group: Joaquin,Jennifer, Lauren, and Hugo 
...
#  first step we were are kind og dividing  the regression in different sub-groups 

# based on the  sub-groups, there is a probability that  they are not now showing off in some  columns so make sure to check summary() of each -- don't want Min = Max = 0!
...
# need [] since model.matrix includes intercept term
#added variable 
...
library(standardize)
sobj <- standardize(vaxx ~ TBIRTH_YEAR + EEDUCsome.hs + EEDUCHS.diploma + EEDUCsome.coll + EEDUCassoc.deg + EEDUCbach.deg + EEDUCadv.deg +  MSmarried + MSwidowed + MSdivorced + MSseparated + MSnever + RRACEBlack + RRACEAsian + RRACEOther +
...
# in this part we came up with a final data witch states that people without college degree  came up with a negative numbers regarding to vaccination. 
#this model is to understand how to call the standardized objects from above and interpret the results. the graph is kind of hard to understand.
require(spikeslab)
set.seed(54321)
